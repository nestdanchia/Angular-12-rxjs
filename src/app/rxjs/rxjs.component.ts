...
import { throwError, of, forkJoin, combineLatest, BehaviorSubject, fromEvent, from, zip } from 'rxjs';
import {  catchError, tap, map, switchMap, filter, first, mergeMap, debounceTime, withLatestFrom } from 'rxjs/operators';
@Component({
  selector: 'app-rxjs',
...
.subscribe(([color, logo]) => console.log(`primera fuente ${color} combineLatest -->shirt with segunda fuente ${logo}`));
